  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Audit Trail - Loan #${bookLoanNumber}`} maxWidth="max-w-2xl">
      {loading ? (
        ...
        <div className="py-8 text-center text-sm text-slate-400">No activity history recorded yet.</div>
      ) : (
        <div className="relative border-l-2 border-slate-200 ml-4 space-y-6 my-2">
              ...
              <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5 space-y-1">
                ...
                {item.event_type === 'transaction' ? (
                  <p className="text-sm font-semibold text-slate-800">
                    Payment of <span className="text-emerald-600">₹{parseFloat(item.amount_paid || 0).toLocaleString()}</span> ({item.payment_type})
                  </p>
                ) : item.field_changed === 'creation' ? (
                  <div className="text-sm font-bold text-amber-950">
                    <span className="text-amber-700">🎉 {item.old_value}:</span>{' '}
                    <span className="font-semibold text-slate-800">{item.new_value}</span>
                  </div>
                ) : (
                  <div className="text-sm text-slate-700">
                    <span className="font-semibold text-slate-800">{item.field_changed}</span> changed from{' '}
                    <span className="line-through text-slate-400">{item.old_value || 'empty'}</span> to{' '}
